...
fn test_pyo3_new_gate_definition() {
    ...
    Python::with_gil(|py| {
        ...
        let copy_param = convert_operation_to_pyobject(input_definition, py)
            .unwrap()
...
fn test_pyo3_input_symbolic_input() {
    ...
    Python::with_gil(|py| {
        ...
        .unwrap();
        let input_op: &f64 = &operation.call_method0("input").unwrap().extract().unwrap();
...
fn test_pyo3_input_bit_index() {
    ...
    Python::with_gil(|py| {
        ...
        .unwrap();
        let input_op: &usize = &operation.call_method0("index").unwrap().extract().unwrap();
...
fn test_pyo3_input_bit_value() {
    ...
    Python::with_gil(|py| {
        ...
        .unwrap();
        let input_op: &bool = &operation.call_method0("value").unwrap().extract().unwrap();
...
fn test_pyo3_remap_qubits_gate_definition() {
    ...
    Python::with_gil(|py| {
        ...
        .unwrap();
        let remaped_operation = convert_operation_to_pyobject(
            Operation::from(GateDefinition::new(
                Circuit::new(),
                String::from("name"),
                vec![1],
