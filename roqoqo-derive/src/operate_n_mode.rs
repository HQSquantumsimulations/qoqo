...
pub fn dispatch_struct_enum_multi_mode(input: DeriveInput) -> TokenStream {
    let ident = input.ident;
    match input.data {
        Data::Struct(_ds) => operate_multi_mode_struct(ident),
        Data::Enum(de) => operate_multi_mode_enum(de, ident),
        _ => panic!("OperateSingleMode can only be derived on structs and enums"),
    }
}

fn operate_multi_mode_enum(de: DataEnum, ident: Ident) -> TokenStream {
    let DataEnum { variants, .. } = de;
    let match_quotes = variants.into_iter().map(|v| {
        let vident = v.ident.clone();
        let fields = match v.fields {
            Fields::Unnamed(fields) => fields,
            _ => panic!(
                "OperateMultiMode can only be derived for enums with newtype structs as variants"
            ),
        };
        if fields.unnamed.iter().len() != 1 {
            panic!(
        ...
        quote! {
            &#ident::#vident(ref inner) => {OperateMultiMode::modes(&(*inner))},
        }
    });
    quote! {
        /// Implements [OperateMultiMode] trait for this Operation acting on a set of bosonic modes.
        #[automatically_derived]
        impl OperateMultiMode for #ident{
            /// Returns bosonic `modes` the Operation acts on.
...
fn operate_multi_mode_struct(ident: Ident) -> TokenStream {
    quote! {
        /// Implements [OperateMultiMode] trait for this Operation acting on a set of bosonic modes.
        #[automatically_derived]
        impl OperateMultiMode for #ident{
